...
final class SvnWorkflowTest extends TestCase {
	...
	public function testFullSvnWorkflowForOneFile() {
		...
		$options = CliOptions::fromArray(['svn' => true, 'files' => [$svnFile]]);
		$expected = $this->phpcs->getResults('bin/foobar.php', [20]);
		$messages = runSvnWorkflow($options, $shell, new CacheManager(new TestCache()), '\PhpcsChangedTests\debug');
	...
	public function testFullSvnWorkflowForOneFileCached() {
		...
		$messages = runSvnWorkflow($options, $shell, new CacheManager($cache), '\PhpcsChangedTests\debug');
		$this->assertEquals($expected->getMessages(), $messages->getMessages());
	...
	public function testFullSvnWorkflowForOneFileUncachedThenCachesBothVersionsOfTheFile() {
		...
		runSvnWorkflow($options, $shell, $manager, '\PhpcsChangedTests\debug');
		...
		$messages = runSvnWorkflow($options, $shell, $manager, '\PhpcsChangedTests\debug');
		$this->assertEquals($expected->getMessages(), $messages->getMessages());
	}
	...
	public function testFullSvnWorkflowForOneFileWithCacheOfOldFileVersionDoesNotUseCache() {
		...
		runSvnWorkflow($options, $shell, new CacheManager($cache), '\PhpcsChangedTests\debug');
	...
	public function testFullSvnWorkflowForMultipleFiles() {
		...
		$options = CliOptions::fromArray(['svn' => true, 'files' => $svnFiles]);
	...
	public function testFullSvnWorkflowForNonSvnFile() {
		...
		$shell->registerCommand("cat 'foobar.php'", $this->phpcs->getResults('STDIN', [20, 99])->toPhpcsJson());
		$options = CliOptions::fromArray(['svn' => true, 'files' => [$svnFile]]);
		runSvnWorkflow($options, $shell, new CacheManager(new TestCache()), '\PhpcsChangedTests\debug');
	}
